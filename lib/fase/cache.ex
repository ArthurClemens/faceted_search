defmodule Fase.Cache do
  ...
  def handle_call({:get, view_name, data_key}, _from, table) do
    result =
      ...
      |> tap(fn
        {:ok, _} ->
          Logger.debug("Fase.Cache.get/3: data read from cache")

        {:error, :no_cache} ->
          Logger.debug("Fase.Cache.get/3: no cache")
      end)
  ...
  def handle_cast({:insert, view_name, data_key, data}, table) do
    ...
    |> tap(fn
      _ ->
        Logger.debug("Fase.Cache.insert/4: data written to cache")
    end)
  ...
  def handle_cast({:clear, view_name}, table) do
    ...
    |> tap(fn _ ->
      Logger.debug("Fase.Cache.clear/2: cache cleared for view: #{view_name}")
    end)
  ...
  def handle_cast({:delete}, table) do
    ets_delete(table)
    |> tap(fn _ ->
      Logger.debug("Fase.Cache.clear/2: cache deleted")
    end)
